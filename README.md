...
6. **Cache has a significant impact.** YMM achieved around 3.2×–3.3× speedup especially during the boundary sizes for small input sizes because the data fits in the CPU cache. For very large inputs, the speedup dropped to about 1.25× since the processor had to access slower main memory.
